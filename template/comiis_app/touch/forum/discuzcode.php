...
function tpl_quote() {
<!--{/eval}-->
<!--{block return}--><div class="comiis_quote bg_h b_dashed f_c"><blockquote><font size="2">{lang reply}</font> \\1</blockquote></div><!--{/block}-->
<!--{eval}-->
	return $return;
}
function tpl_free() {
...
<!--{block return}-->
	<!--{if $attach['attachimg'] && $_G['setting']['showimages'] && (($_G['group']['allowgetimage'] || $_G['uid'] == $attach['uid']) || (($guestviewthumb)))}-->
		<!--{if $guestviewthumb}-->
			<!--{if $post['first'] == 0 && $comiis_app_switch['comiis_aimg_show'] == 1 || $comiis_app_switch['comiis_aimg_oneshow'] == 1 && $post['first'] == 1}-->
			<li><a href="javascript:;"{if !$_G[connectguest]} onclick="popup.open('{$comiis_lang['tip8']}', 'confirm', 'member.php?mod=logging&action=login');"{else} onclick="popup.open('{$comiis_lang['reg23']}', 'confirm', 'member.php?mod=connect');"{/if}><img id="aimg_$attach[aid]" {if $comiis_app_switch['comiis_loadimg']}src="https://cdn.jsdelivr.net/gh/jingjiniao-info/cdn/template/comiis_app/pic/none.png" comiis_loadimages={else}src={/if}"$mobilethumburl" alt="{if $attach['description']}$attach['description']{else}{$_G['forum_thread']['subject']}{/if}[{$attach[imgalt]}]"{if $comiis_app_switch['comiis_loadimg']} class="comiis_loadimages"{/if} /></a></li>
			<!--{else}-->
			<div class="comiis_nouidpic">
				<a href="javascript:;"{if !$_G[connectguest]} onclick="popup.open('{$comiis_lang['tip8']}', 'confirm', 'member.php?mod=logging&action=login');"{else} onclick="popup.open('{$comiis_lang['reg23']}', 'confirm', 'member.php?mod=connect');"{/if}><img id="aimg_$attach[aid]" {if $comiis_app_switch['comiis_loadimg']}src="https://cdn.jsdelivr.net/gh/jingjiniao-info/cdn/template/comiis_app/pic/none.png" comiis_loadimages={else}src={/if}"$mobilethumburl" alt="{if $attach['description']}$attach['description']{else}{$_G['forum_thread']['subject']}{/if}[{$attach[imgalt]}]" class="vm{if $comiis_app_switch['comiis_loadimg']} comiis_loadimages{/if}" /></a>
			...
			<!--{/if}-->
		<!--{else}-->
			<li><span onclick="window.location.href='forum.php?mod=viewthread&tid=$attach[tid]&aid=$attach[aid]&from=album&page=$_G[page]'"><img id="aimg_$attach[aid]" {if $comiis_app_switch['comiis_loadimg']}src="https://cdn.jsdelivr.net/gh/jingjiniao-info/cdn/template/comiis_app/pic/none.png" comiis_loadimages={else}src={/if}"$mobilethumburl" alt="{if $attach['description']}$attach['description']{else}{$_G['forum_thread']['subject']}{/if}[{$attach[imgalt]}]"{if $comiis_app_switch['comiis_loadimg']} class="comiis_loadimages"{/if} /></span></li>
		<!--{/if}-->
	<!--{/if}-->
<!--{/block}-->
<!--{eval}-->
...
function attachinpost($attach, $post) {
	...
	if($guestviewthumb){
		...
		$mobilethumburl = $attach['attachimg'] && $_G['setting']['showimages'] && (!$attach['price'] || $attach['payed']) && ($_G['group']['allowgetimage'] || $_G['uid'] == $attach['uid']) ? ($comiis_app_switch['comiis_view_imgdata'] == 1 ? $attach[url].$attach[attachment] : getforumimg($attach['aid'], 0, 600, 1000, 'fixnone')) : getforumimg($attach['aid'], 0, 160, 300, 'fixnone') ;
	}	
	$is_archive = $_G['forum_thread']['is_archived'] ? '&fid='.$_G['fid'].'&archiveid='.$_G[forum_thread][archiveid] : '';
	$attach['refcheck'] = (!$attach['remote'] && $_G['setting']['attachrefcheck']) || ($attach['remote'] && ($_G['setting']['ftp']['hideurl'] || ($attach['isimage'] && $_G['setting']['attachimgpost'] && strtolower(substr($_G['setting']['ftp']['attachurl'], 0, 3)) == 'ftp')));
	$aidencode = packaids($attach);
...
<!--{block return}-->
	<!--{if $attach['attachimg'] && $_G['setting']['showimages'] && (((!$attach['price'] || $attach['payed']) && ($_G['group']['allowgetimage'] || $_G['uid'] == $attach['uid'])) || (($guestviewthumb)))}-->
		<!--{if $guestviewthumb}-->
		<div class="comiis_nouidpic">
			<a href="javascript:;"{if !$_G[connectguest]} onclick="popup.open('{$comiis_lang['tip8']}', 'confirm', 'member.php?mod=logging&action=login');"{else} onclick="popup.open('{$comiis_lang['reg23']}', 'confirm', 'member.php?mod=connect');"{/if}><img id="aimg_$attach[aid]" {if $comiis_app_switch['comiis_loadimg']}src="https://cdn.jsdelivr.net/gh/jingjiniao-info/cdn/template/comiis_app/pic/none.png" comiis_loadimages={else}src={/if}"$mobilethumburl" alt="$attach[imgalt]" title="$attach[imgalt]" class="vm{if $comiis_app_switch['comiis_loadimg']} comiis_noloadimage comiis_loadimages{/if}" /></a>
			<div class="comiis_nouidpic_tip f_ok"><a href="javascript:;"{if !$_G[connectguest]} onclick="popup.open('{$comiis_lang['tip8']}', 'confirm', 'member.php?mod=logging&action=login');"{else} onclick="popup.open('{$comiis_lang['reg23']}', 'confirm', 'member.php?mod=connect');"{/if}>{lang guestviewthumb}</a></div>
		</div>
		<!--{else}-->
		<span onclick="window.location.href='forum.php?mod=viewthread&tid=$attach[tid]&aid=$attach[aid]&from=album&page=$_G[page]'" class="comiis_postimg vm"><img id="aimg_$attach[aid]" {if $comiis_app_switch['comiis_loadimg']}src="https://cdn.jsdelivr.net/gh/jingjiniao-info/cdn/template/comiis_app/pic/none.png" comiis_loadimages={else}src={/if}"$mobilethumburl" alt="$attach[imgalt]" title="$attach[imgalt]"{if $comiis_app_switch['comiis_loadimg']} class="comiis_loadimages"{/if} /></span>			
		<!--{/if}-->				
	<!--{else}-->
	<div class="comiis_attach bg_e b_ok cl">
		<a {if $_G['uid']}{if !$attach['price'] || $attach['payed']}href="forum.php?mod=attachment{$is_archive}&aid=$aidencode"{else}href="forum.php?mod=misc&action=attachpay&aid=$attach[aid]&tid=$attach[tid]" class="dialog"{/if}{else}href="javascript:;" class="comiis_openrebox"{/if}>
