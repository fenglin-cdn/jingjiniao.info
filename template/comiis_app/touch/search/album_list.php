...
<!--{if !empty($albumlist)}-->
	<div class="comiis_p12 f14 bg_f f_c b_b cl" style="padding-bottom:10px;><!--{if $keyword}-->{lang search_result_keyword}<!--{else}-->{lang search_result}<!--{/if}--></div>
<!--{/if}-->
<!--{if empty($albumlist)}-->
		...
		<ul>
			...
			<li>
				<a href="home.php?mod=space&uid=$value[uid]&do=album&id=$value[albumid]">
					<!--{if $value[pic]}--><img src="{echo str_replace(array('static/image/common/nopublish.jpg'), array('https://cdn.jsdelivr.net/gh/jingjiniao-info/cdn/template/comiis_app/comiis/img/comiis_jmalbum.jpg'), $value[pic]);}" class="vm" /><!--{else}--><img src="{IMGDIR}/comiis_noalbum.jpg" class="vm" /><!--{/if}-->
