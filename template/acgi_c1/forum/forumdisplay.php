...
<div class="boardnav">
	<div id="ct" class="wp cl{if $_G['forum']['allowside']} ct2{/if}"{if $leftside} style="margin-left:{$_G['leftsidewidth_mwidth']}px"{/if}>
		...
		<div class="mn{if $leftside} y{/if}">
			<div class="bm bml acpbn">
				<!--{if $_G[forum][banner] && !$subforumonly}--><img src="$_G[forum][banner]" alt="$_G['forum'][name]" /><!--{/if}-->
				<div class="bm_h cl">
					<!--{if $_G['page'] == 1 && $_G['forum']['rules']}--><span class="o"><img id="forum_rules_{$_G[fid]}_img" src="$_G['style'][styleimgdir]/common/collapsed_{$collapse[forum_rulesimg]}.gif" title="{lang spread}" alt="{lang spread}" onclick="toggle_collapse('forum_rules_{$_G[fid]}')" /></span><!--{/if}-->
					<span class="y">
			...
			<!--{if !$subforumonly}-->
				...
				<div class="bm bmw fl{if $_G['forum']['forumcolumns']} flg{/if} acpbn">
					<div class="bm_h cl">
				...
				<!--{if $livethread}-->
					<div id="livethread" class="tl bm bmw acpbn cl" style="padding:10px 15px;">
				...
				<div id="pgt" class="bm bw0 pgs cl">
					...
					<!--{if !$_GET['archiveid']}--><a href="javascript:;" id="newspecial" class="acpost" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if}>{lang send_posts}</a><!--{/if}-->
		...
		<div class="sd">
			...
			<div class="bm bmbd">
				<div class="bm_c">
					<!--[diy=fdsd_slide]--><div id="fdsd_slide" class="area"></div><!--[/diy]-->
				</div>
			</div>
			...
			<!--{if !$subforumonly}-->
				<div class="bm">
					...
					<div class="bm_c">
						<ul class="c1sd_cat cl">
						<!--{loop $_G['cache']['forums'] $bforum}-->
							<!--{if $bforum['fup'] == $_G['forum']['fup'] && $bforum['status']}-->
								<li><a href="forum.php?mod=forumdisplay&fid=$bforum[fid]" class="txt_s" title="{$bforum['name']}">$bforum['name']</a></li>
							<!--{/if}-->
						<!--{/loop}-->
						</ul>
					</div>
				...
				<!--{if !($_G['forum']['simple'] & 1) && $_G[setting][whosonlinestatus]}-->
					<div class="bm">
						<!--{if $detailstatus}-->
						<div class="bm_h cl">
							<span class="o y"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]&page=$page&showoldetails=no#online"><img src="$_G['style'][styleimgdir]/common/collapsed_no.gif" alt="" /></a></span>
						...
						<!--{else}-->
						<div class="bm_h cl">
							<span class="o y"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]&page=$page&showoldetails=yes#online" class="nobdr"><img src="$_G['style'][styleimgdir]/common/collapsed_yes.gif" alt="" /></a></span>
							<h2>{lang forum_activeusers} ($onlinenum)</h2>
						</div>
						<!--{/if}-->
					</div>
			...
			<div class="bm">
				<div class="bm_h">
					<h2>{$c1lang[hot_rec]}</h2>
				</div>
				<div class="bm_c">
					<!--[diy=fdsd_rec]--><div id="fdsd_rec" class="area"></div><!--[/diy]-->
				</div>
			</div>
			<div class="bm">
				<div class="bm_h">
				...
				<div class="bm_c">
					<div class="c1sd_ranktab cl">
						<ul><li class="cur">{$c1lang[day_rank]}</li><li>{$c1lang[week_rank]}</li><li>{$c1lang[month_rank]}</li></ul>
					</div>
					<div class="c1sd_ranklist">
						<div style="display:block;">
							<!--[diy=fdsd_day_rank]--><div id="fdsd_day_rank" class="area"></div><!--[/diy]-->
						</div>
						<div>
							<!--[diy=fdsd_week_rank]--><div id="fdsd_week_rank" class="area"></div><!--[/diy]-->
						</div>
						<div>
							<!--[diy=fdsd_month_rank]--><div id="fdsd_month_rank" class="area"></div><!--[/diy]-->
						</div>
					</div>
				</div>
			</div>
			...
			<script type="text/javascript">
				jq(".c1sd_bmslide").slide({titCell:".hd ul",mainCell:".bd ul",autoPage:true,effect:"fold",autoPlay:true,trigger:"click",interTime:3000});
				jq('.c1sd_ranktab ul li').each(function(s){
					jq(this).click(function(){
						jq(this).addClass('cur').siblings().removeClass('cur');
						jq('.c1sd_ranklist > div').eq(s).show(0).siblings().hide();
					})
				})
			</script>
			<div class="drag">
