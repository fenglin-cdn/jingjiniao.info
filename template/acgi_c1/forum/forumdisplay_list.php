<?php echo ' ';exit;?>
<div id="threadlist" class="tl bm acpbn bmw_0"{if $_G['uid']} style="position: relative;"{/if}>
				...
				<!--{if CURMODULE != 'guide'}-->
					<div class="tf">
						...
						<!--{if empty($_G['forum']['picstyle']) && $_GET['orderby'] == 'lastpost' && (!$_G['setting']['forumseparator'] || !$separatepos) && !$_GET['filter']}-->
							<a href="javascript:;" onclick="checkForumnew_btn('{$_G['fid']}')" title="{lang showupgrade}" class="forumrefresh rotate360"></a>
						<!--{/if}-->
						<!--{if $_GET['filter'] == 'hot'}-->
		...
		<form method="post" autocomplete="off" name="moderate" id="moderate" action="forum.php?mod=topicadmin&action=moderate&fid=$_G[fid]&infloat=yes&nopost=yes">
			...
			<table summary="forum_$_G[fid]" cellspacing="0" cellpadding="0" id="threadlisttableid">
				<!--{if (!$simplestyle || !$_G['forum']['allowside'] && $page == 1) && !empty($announcement)}-->
					<tbody>
						<tr>
							<td class="icn"><div class="threadann"><img src="$_G['style'][styleimgdir]/ann_icon.png" alt="{lang announcement}" /></div></td>
							<!--{if $_G['forum']['ismoderator'] && !$_GET['archiveid']}--><td class="o">&nbsp;</td><!--{/if}-->
							<th>
								<div class="acgiflists" style="width:100%;">
									<div class="acgiflisttitle">
										<span class="xst">{lang announcement}: <!--{if empty($announcement['type'])}--><a href="forum.php?mod=announcement&id=$announcement[id]#$announcement[id]" target="_blank">$announcement[subject]</a><!--{else}--><a href="$announcement[message]" target="_blank">$announcement[subject]</a><!--{/if}--></span>
									</div>
									<div class="acgiflistinfo">
										<div class="acgifby1">
										{$c1lang[post]}: <a href="home.php?mod=space&uid=$announcement[authorid]" cs="1">$announcement[author]</a>
				...
				<!--{if $_G['forum_threadcount']}-->
							...
							<!--{if $_G[setting][forumseparator] == 1 && $separatepos == $key + 1}-->
								<tbody id="separatorline">
									<tr class="ts">
										...
										<th><!--{if empty($_G['forum']['picstyle']) && $_GET['orderby'] == 'lastpost' && !$_GET['filter']}--><a href="javascript:;" onclick="checkForumnew_btn('{$_G['fid']}')" title="{lang showupgrade}" class="forumrefresh">{lang forum_thread}&nbsp;&nbsp;</a><!--{else}--><em class="norefresh">&nbsp;</em><!--{/if}--></th><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>
									</tr>
								</tbody>
								...
								<tr>
									<td class="icn">
										<div class="threadavr">
                                            ...
                                            <!--{else}-->
                                                <img src="$_G['style']['styleimgdir']/noavatar_middle.gif" title="$_G[setting][anonymoustext]" alt="$_G[setting][anonymoustext]" />
                                            <!--{/if}-->
										</div>
									</td>
									...
									<th class="$thread[folder]">
										...
										<!--{/if}-->
                                            ...
                                            <div class="acgiflistinfo">
                                                <div class="acgifby1">
                                                    ...
                                                    <!--{if $thread['authorid'] && $thread['author']}-->
                                                        {lang thread_author}:&nbsp;<a href="home.php?mod=space&uid=$thread[authorid]" cs="1" {if $groupcolor[$thread[authorid]]} style="color: $groupcolor[$thread[authorid]];"{/if}>$thread[author]</a><!--{if !empty($verify[$thread['authorid']])}--> $verify[$thread[authorid]]<!--{/if}-->
                                                    <!--{else}-->
                                                ...
                                                <b class="acgib">|</b>
                                                <div class="acgifby2">
                                                    {lang forum_lastpost}:&nbsp;<!--{if $thread['lastposter']}--><a href="{if $thread[digest] != -2}home.php?mod=space&username=$thread[lastposterenc]{else}forum.php?mod=viewthread&tid=$thread[tid]&page={echo max(1, $thread[pages]);}{/if}" cs="1">$thread[lastposter]</a><!--{else}-->$_G[setting][anonymoustext]<!--{/if}-->
                                    	...
                                    	<div class="acgifnums">
                                        	<a href="forum.php?mod=viewthread&tid=$thread[tid]&extra=$extra" class="xi2">$thread[allreplies]</a>&nbsp;/
                                        	<span><!--{if $thread['isgroup'] != 1}-->$thread[views]<!--{else}-->{$groupnames[$thread[tid]][views]}<!--{/if}--></span>
                                            <div class="clear"></div>
                                        </div>
                                    	<div class="clear"></div>
					...
					<!--{else}-->
						...
						<ul id="waterfall" class="ml waterfall cl">
							...
							<!--{eval $waterfallwidthgd = 205; }--><!--li width-->
							<li style="width:{$waterfallwidth}px">
								...
								<h3 class="xw0">
									<!--{hook/forumdisplay_thread $key}-->
									<a href="forum.php?mod=viewthread&tid=$thread[tid]&{if $_GET['archiveid']}archiveid={$_GET['archiveid']}&{/if}extra=$extra"$thread[highlight]{if $thread['isgroup'] == 1 || $thread['forumstick']} target="_blank"{else} onclick="atarget(this)" title="$thread[subject]"{/if}>$thread[subject]</a>
								</h3>
								<div class="auth cl">
									<cite class="xg1 y">
									...
									<!--{hook/forumdisplay_author $key}-->
									<!--{if $thread['authorid'] && $thread['author']}-->
										<a href="home.php?mod=space&uid=$thread[authorid]"><i class="avr"><!--{avatar($thread[authorid],small)}--></i>$thread[author]</a><!--{if !empty($verify[$thread['authorid']])}--> $verify[$thread[authorid]]<!--{/if}-->
									<!--{else}-->
										$_G[setting][anonymoustext]
									<!--{/if}-->
								</div>
													...
													var waitingtimer = setInterval(function () {
														...
														if(loadready && stopload < 2) {
															if(!tmpelems.length) {
																...
																pgbtn.innerHTML = "{lang next_page}";
																showready = 1;
...
<!--{if $multipage && $filter != 'hot'}-->
	<!--{if !($_G[forum][picstyle] && !$_G[cookie][forumdefstyle])}-->
		<a class="bm_h" href="javascript:;" rel="$multipage_more" curpage="$page" id="autopbn" totalpage="$maxpage" picstyle="$_G[forum][picstyle]" forumdefstyle="$_G[cookie][forumdefstyle]">{lang next_page}</a>
		<script type="text/javascript" src="{$_G[setting][jspath]}autoloadpage.js?{VERHASH}"></script>
	<!--{else}-->
		<div id="pgbtn" class="pgbtn"><a href="forum.php?mod=forumdisplay&fid={$_G[fid]}&filter={$filter}&orderby={$_GET[orderby]}{$forumdisplayadd[page]}&{$multipage_archive}&page=$nextpage" hidefocus="true">{lang next_page}</a></div>
	<!--{/if}-->
<!--{/if}-->
<div class="bm bw0 pgs cl">
	...
	<!--{if !$_GET['archiveid']}--><a href="javascript:;" id="newspecialtmp" class="acpost" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} title="{lang send_posts}">{lang post_newthread}</a><!--{/if}-->
