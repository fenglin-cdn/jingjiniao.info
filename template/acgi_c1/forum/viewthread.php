...
<div id="ct" class="wp cl">
	<div id="pgt" class="pgs mbm cl {if $modmenu['thread']}pbm b/bs{/if}">
		...
		<!--{if $_G['forum']['threadsorts'] && $_G['forum']['threadsorts']['templatelist']}-->
			<!--{loop $_G['forum']['threadsorts']['types'] $id $name}-->
				<button id="newspecial" class="acpost" onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]&extra=$extra&sortid=$id'"><strong>{lang i_want}$name</strong></button>
			<!--{/loop}-->
		<!--{else}-->
			<!--{if !$_G['forum_thread']['is_archived']}--><a id="newspecial" class="acpost" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} href="javascript:;">{lang send_posts}</a><!--{/if}-->
		<!--{/if}-->
		<!--{if $allowpostreply && !$_G['forum_thread']['archiveid']}-->
			<a id="post_reply" class="acpost" onclick="showWindow('reply', 'forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]')" href="javascript:;">{lang reply}</a>
		<!--{/if}-->
...
<!--{if $modmenu['thread']}-->
	<div id="acgi_modmenu_menu" class="p_pop" style=" display: none;"  ctrlid="acgi_modmenu" ctrlclass="hover">
			<!--{eval $modopt=0;}-->
		<!--{if $_G['forum']['ismoderator']}-->
	...
	<table cellspacing="0" cellpadding="0">
		<tr>
			...
			<td class="plc ptm pbn vwthd">
				<!--{if !IS_ROBOT}-->
					<div class="y">
						<!--{if $post['invisible'] == 0}--><a href="" class="c1_print" title="{lang thread_printable}" target="_blank">P</a>
						<!--{/if}-->
						<!--{if $modmenu['thread']}-->
							<a href="javascript:;" id="acgi_modmenu" onmouseover="showMenu({'ctrlid':'acgi_modmenu','ctrlclass':'hover','duration':2});">MOD</a>
							<script type="text/javascript">
								var modtop = jq("#acgi_modmenu").offset().top - 40;
								jq(window).scroll(function() {
									if (jq(this).scrollTop() > modtop){
										jq("#acgi_modmenu").css("position","fixed");
										jq("#acgi_modmenu").css("top",50);
									}else{
										jq("#acgi_modmenu").css("position","static");
										jq("#acgi_modmenu").css("top",0);
									}
								})
							</script>
						<!--{/if}-->
...
<div class="pgs mtm mbm cl">
	...
	<!--{if !$_G['forum_thread']['is_archived']}-->
		<a id="newspecialtmp" class="acpost" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} href="javascript:;">{lang send_posts}</a>
	<!--{/if}-->
	<!--{if $allowpostreply && !$_G['forum_thread']['archiveid']}-->
		<a id="post_replytmp" class="acpost" onclick="showWindow('reply', 'forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]')" href="javascript:;">{lang reply}</a>
	<!--{/if}-->
</div>
